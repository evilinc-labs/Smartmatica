...
public class SchematicPrinter {
    ...
    private void tickBuilding(MinecraftClient mc) {
        ...
        // allowPlace enabled (pillaring, bridging, parkour).
        {
            ...
            if (nearbyZone != null) {
                ...
                int maxR = (int) Math.ceil(range);
                if (nearbyZone.getY() > pY + maxR) {
                    int radius = (int) Math.ceil(range);
                    walkToZoneWithPlacement(mc.player, nearbyZone, radius);
                    autoState = AutoState.WALKING_TO_BUILD;
        ...
        if (!tryWalkToNextZone(mc)) {
            ...
            if (!tryWalkToNextZone(mc)) {
                ...
                PrinterDatabase.clearSnapshots(); // clear stale chest snapshots
    ...
    private boolean tryWalkToNextZone(MinecraftClient mc) {
        ...
        else if (standPos != null && standPos.getY() > playerY + 2) {
            needsClimbing = true;
        }
        // 3. No standing position exists and target is above the player
        else if (standPos == null && targetY > playerY + 2) {
            needsClimbing = true;
        }

        if (needsClimbing) {
            // Let Baritone handle vertical movement (pillar, bridge, parkour)
    ...
    private void tickRestocking(MinecraftClient mc) {
        ...
        if (handler instanceof GenericContainerScreenHandler containerHandler) {
            ...
            if (supplyTarget != null) {
                PrinterDatabase.invalidateSnapshot(supplyTarget);
            }
    ...
    private List<BlockPos> computeSupplyWaypoints(BlockPos from, BlockPos to) {
        ...
        for (BlockPos chest : PrinterDatabase.getChestPositions()) {
    ...
    private void walkToZoneWithPlacement(ClientPlayerEntity player,
        ...
        if (vertDist > 8 && dy > 0) {
            ...
            // ascent > 8 blocks needs this to avoid mid-air waypoints.
            if (horizDist > 4) {
    ...
    private BlockPos findStandingPosition(BlockPos target, World world, ClientPlayerEntity player) {
        ...
        // Search at y-offsets from -4 to +4 relative to the target
        // so the player can stand above, below, or at the same level.
        // Range check still ensures the block is within placement reach.
            ...
            for (int dx = -maxReach; dx <= maxReach; dx++) {
                for (int dz = -maxReach; dz <= maxReach; dz++) {
                    ...
                    *//*?} else {*/
                    ...
                    double penalty = inWater ? 50.0 : 0.0;

                    // Heavily penalise positions separated from the player
                    // by an air gap (no continuous ground between them).
                    // This prevents picking standing spots across chasms
                    // that Baritone can't walk to.
